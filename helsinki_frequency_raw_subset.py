...
FULL_INPUT_SIZE = N_CHANNELS   # 8 — raw signal, no subband decomposition

CHECKPOINT_DIR = "/content/drive/MyDrive/checkpoints"
FEATURE_CACHE  = "/content/drive/MyDrive/checkpoints/loo_features_v7.pkl"   # v7 = raw signal
RESUME_PATH    = "/content/drive/MyDrive/checkpoints/loo_resume.pkl"
...
def compute_raw_sequence(data, fs=_FS):
    """
    (n_ch, n_samp) -> (T_seq, n_ch)
    Pure raw signal resampled to T_seq timepoints.
    Each timestep t contains one amplitude value per channel.
    """
    raw_resampled = resample(data, T_SEQ, axis=1)   # (n_ch, T_seq)
    return raw_resampled.T.astype(np.float32)        # (T_seq, n_ch)
...
def load_recording(file_path):
    ...
    try:
        ...
        data = (data - data.mean(axis=1, keepdims=True)) / \
               (data.std(axis=1, keepdims=True) + 1e-8)    # per-channel z-score
...
def build_all_features(data_path, annotations_path):
    ...
    Returns list of dicts, one per valid recording:
      { 'rec_idx', 'file', 'X': (n_win, T_seq, 8), 'y': (n_win,) }
    z-score applied per LOO fold to avoid leakage — not here.
    ...
    for edf_file in tqdm(edf_files, desc="Pre-computing features (runs once)"):
        ...
        for start in range(0, len(annot) - 1, step_samp):
            ...
            if window.shape[1] < win_samp:
                window = np.concatenate(
                    ...
                    axis=1)
            ps = compute_raw_sequence(window)
        ...
        recs.append({
            'rec_idx': col_idx,
            'file':    edf_file.name,
            'X':       np.stack(X_list).astype(np.float32),   # (n_win, T_seq, 8)
...
# ============================================================
# Models — all take (B, T_seq, 8)
# ============================================================

class DIRUCell(nn.Module):
    """
    K compartments, each with its own W_in over the full input.
    Inductive bias: K separate learned projections.
    """
...
# ============================================================
# LOO loop — with resume checkpointing
# ============================================================

def run_loo(recordings):
    """
    Leave-one-recording-out CV.
    For each fold i: train on n-1 recordings, test on 1.
    z-score fitted on train windows only, applied to test — no leakage.
    Progress saved after every fold; resumes automatically if interrupted.
    """
    n                  = len(recordings)
    ...
    # ── Resume: reload any previously saved fold results ──
    resume_path = Path(RESUME_PATH)
    if resume_path.exists():
        resume             = pickle.load(open(resume_path, 'rb'))
        all_probs          = resume['all_probs']
        all_labels         = resume['all_labels']
        completed_folds    = resume['completed_folds']
        single_class_folds = resume['single_class_folds']
        print(f"Resuming from fold checkpoint: "
              f"{len(completed_folds)}/{n} folds already complete")

    for fold_i in tqdm(range(n), desc="LOO folds"):
        if fold_i in completed_folds:
            continue   # skip already-finished folds

        test_rec   = recordings[fold_i]
        train_recs = [recordings[j] for j in range(n) if j != fold_i]

        X_tr = np.concatenate([r['X'] for r in train_recs], axis=0)  # (N, T_seq, C)
        y_tr = np.concatenate([r['y'] for r in train_recs], axis=0)
        X_te = test_rec['X']                                          # (M, T_seq, C)
        y_te = test_rec['y']

        if len(np.unique(y_te)) < 2:
            single_class_folds += 1
            # Still include — they contribute TN/TP to pooled eval

        # z-score: fit on train, apply to test
        # reshape to (N*T_seq, C), compute per-channel stats, broadcast back
        flat = X_tr.reshape(-1, FULL_INPUT_SIZE)   # (N*T_seq, C)
        mu   = flat.mean(axis=0)                   # (C,)
        std  = flat.std( axis=0)                   # (C,)
        X_tr_n = (X_tr - mu) / (std + 1e-8)       # broadcasts over (N, T_seq, C)
        ...
        pickle.dump({
            ...
            'single_class_folds':  single_class_folds,
        }, open(resume_path, 'wb'))
...
def plot_roc(summary, save_path=None):
    ...
    plt.title('LOO-CV ROC — Neonatal Seizure (Raw Signal v7)', fontsize=13)
...
def run(data_path, clear_cache=False):
    ...
    rpath = Path(CHECKPOINT_DIR) / 'loo_results_v7.pkl'
    pickle.dump({'probs': all_probs, 'labels': all_labels, 'summary': summary},
                open(rpath, 'wb'))
    print(f"Results saved: {rpath}")

    # Clean up resume checkpoint now that we have final results
    resume_path = Path(RESUME_PATH)
    if resume_path.exists():
        resume_path.unlink()
        print("Resume checkpoint cleaned up")

    plot_roc(summary, save_path=Path(CHECKPOINT_DIR) / 'roc_loo_v7.png')
    return summary


if __name__ == "__main__":
    DATA_PATH   = "/content/drive/MyDrive/eeg_cache"
    CLEAR_CACHE = True   # set True on first run to rebuild cache with raw signal pipeline
